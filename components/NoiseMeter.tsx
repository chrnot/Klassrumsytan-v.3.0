...
const NoiseMeter: React.FC = () => {
  ...
  const startListening = async () => {
    try {
      ...
      // Explicit typning för att undvika ArrayBuffer-matchningsfel
  ...
  const updateLevel = () => {
    ...
    // Använd Uint8Array direkt, men var beredd på miljöer med strängare krav
    const data = dataArrayRef.current;
    analyserRef.current.getByteFrequencyData(data);
    
    // Fix for TS2345/TS2362: Explicitly type reduce arguments as number and specify result type to fix sum calculation
    // Added <number> generic to reduce to ensure 'sum' is inferred as a number and can be used on line 90.
    const sum = Array.from(data).reduce<number>((a: number, b: number) => a + b, 0);
  ...
  return (
    <div className="flex flex-col items-center h-full">
      ...
      <div className="relative flex-1 w-full bg-slate-50 rounded-2xl border border-slate-100 overflow-hidden flex items-end p-2 min-h-[150px]">
        ...
        <div className={`w-full transition-all duration-75 ease-out rounded-xl relative z-10 ${status.bar}`} style={{ height: `${level}%` }}></div>
      </div>
      <div className="mt-4 w-full">
        {!isListening ? (
          <button onClick={startListening} className="w-full py-3 bg-indigo-600 text-white rounded-xl font-bold text-sm hover:bg-indigo-700 shadow-lg shadow-indigo-100">Starta mätning</button>
        ) : (
          <button onClick={stopListening} className="w-full py-3 bg-red-100 text-red-600 rounded-xl font-bold text-sm hover:bg-red-200 transition-all">Stoppa</button>
        )}
      </div>
      <p className="mt-3 text-[9px] text-slate-400 font-medium text-center italic">Varningar ljuder automatiskt vid höga nivåer.</p>
    </div>
  );
};
